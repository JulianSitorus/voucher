...
<body>
    ...
    <div class="sidebar-main">
        <div class="main">            
            ...
            <h1 style="font-size: 22px"><i class="fa-solid fa-pen"></i> Edit Voucher</h1>
            <div class="separator"></div>
            <!-- <button class="back" onclick="location.href='dashboard'">Back</button> -->

                <form action="{{$voucher->id}}" method="POST" enctype="multipart/form-data">
                @method('put')
                @csrf

                    <!-- nama -->
                    <input id="nama" type="text" name="nama" pattern=".*\S+.*" placeholder=" Name" required value="{{$voucher->nama}}"
                    ...
                    onInput="this.setCustomValidity('')" title="Silahkan pilih kategori">
                        ...
                        <option value="Food" @if ($voucher->kategori == "Food") selected @endif>Food</option>
                        <option value="Fashion" @if ($voucher->kategori == "Fashion") selected @endif>Fashion</option>
                        <option value="Electronic" @if ($voucher->kategori == "Electronic") selected @endif>Electronic</option> 
                        <option value="Travelling" @if ($voucher->kategori == "Travelling") selected @endif>Travelling</option> 
                    </select><br>

                    <!-- status -->
                    <input id="status" type="text" name="status" pattern=".*\S+.*" placeholder=" Status" required value="{{$voucher->status}}"
                    ...
                    <p style="margin-left:20px;">*No need to upload photos if there are no changes</p>  
